...
int main( int argc, char *argv[] )
{
	...
	#ifdef TARGET_PS2
	scr_printf("PS2: Calling SDL_Init...\n");
#endif
	if (SDL_Init(0))
	{
	...
	#ifdef TARGET_PS2
	scr_printf("PS2: SDL_Init OK\n");
	// pad init moved to later
#endif
	#ifdef TARGET_PS2
	scr_printf("PS2: Loading config...\n");
#endif
	...
	#ifdef TARGET_PS2
	scr_printf("PS2: Scanning episodes...\n");
#endif
	JE_scanForEpisodes();

	#ifdef TARGET_PS2
	scr_printf("PS2: Config loaded, init video...\n");
#endif
	#ifdef TARGET_PS2
	scr_printf("PS2: Init video...\n");
	sleep(2);
#endif
	init_video();
#ifdef TARGET_PS2
	ps2_log("init_video OK");
	{
		SDL_Surface *vs = SDL_GetVideoSurface();
		char buf[128];
		if (vs) sprintf(buf, "video surface: %dx%dx%d", vs->w, vs->h, vs->format->BitsPerPixel);
		else sprintf(buf, "video surface: NULL!");
		ps2_log(buf);
	}
#endif
	#ifdef TARGET_PS2
	scr_printf("PS2: Video OK\n");
#endif
	#ifdef TARGET_PS2
	ps2_log("before init_keyboard");
#endif
	init_keyboard();
#ifdef TARGET_PS2
	ps2_log("init_keyboard OK");
#endif
	init_joysticks();
#ifdef TARGET_PS2
	ps2_log("init_joysticks OK");
#endif
	...
	#ifdef TARGET_PS2
	ps2_log("before JE_loadPals");
#endif
	JE_loadPals();
	#ifdef TARGET_PS2
	ps2_log("before JE_loadMainShapeTables");
#endif
	...
	{
		#ifdef TARGET_PS2
	ps2_log("before init_audio");
#endif
	printf("initializing SDL audio...\n");

		init_audio();
#ifdef TARGET_PS2
		ps2_log("init_audio OK");
#endif

		load_music();
#ifdef TARGET_PS2
		ps2_log("load_music OK");
#endif

		#ifdef TARGET_PS2
		ps2_log("before JE_loadSndFile");
#endif
	...
	#ifdef TARGET_PS2
	ps2_log("before JE_loadExtraShapes");
#endif
	JE_loadExtraShapes();  /*Editship*/

	#ifdef TARGET_PS2
	ps2_log("before JE_loadHelpText");
#endif
	JE_loadHelpText();
	...
	if (!isNetworkGame)
		#ifdef TARGET_PS2
	ps2_log("before intro_logos");
	ps2_pad_init();
	ps2_pad_activate();
	ps2_log("pad activated");
#endif
	//intro_logos(); // skip on PS2 for now
#endif

	for (; ; )
	{
		#ifdef TARGET_PS2
		ps2_log("before JE_initPlayerData");
#endif
		JE_initPlayerData();
		JE_sortHighScores();
