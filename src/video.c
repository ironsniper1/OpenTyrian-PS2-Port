...
#ifdef TARGET_PS2
	SDL_Surface *surface = SDL_SetVideoMode(320, 200, 16, SDL_SWSURFACE);
	if (surface == NULL) {
		surface = SDL_SetVideoMode(640, 448, 16, SDL_SWSURFACE);
	}
	if (surface == NULL) {
		init_scr();
		scr_printf("PS2: Video mode failed: %s\n", SDL_GetError());
		while(1){}
	}
	scaler = 0;
	scaler_function = scalers[0].scaler16;
	return;
#endif
...
#ifdef TARGET_PS2
	static int vga_log=0;
	if(vga_log<5){ps2_log("showVGA");vga_log++;}
#endif
	scale_and_flip(VGAScreen);
#ifdef TARGET_PS2
	if(vga_log<10){ps2_log("showVGA done");vga_log++;}
#endif
}
